...
window.handleShippingTierChange = function() {
  ...
  if (!tierInput || !select) return;

  select.innerHTML = '';
  LOGISTICS_CONFIG[tierInput.value].forEach(option => {
...
window.handleShippingMethodChange = function() {
  ...
  const noticeElement = document.getElementById('shipping-notice-text');
  ...
  const warningElement = document.getElementById('fee-adjust-warn');
  if (!tierInput || !select || !container || !noticeElement || !feeDisplay || !finalTotal || !warningElement) return;

  const methodConfig = LOGISTICS_CONFIG[tierInput.value].find(item => item.id === select.value);
  if (!methodConfig) return;

  noticeElement.innerText = `💡 ${methodConfig.notice}`;
  currentShippingFee = methodConfig.fee;
  feeDisplay.innerText = methodConfig.fee > 0 ? `$${methodConfig.fee}` : '待後台核定';
  warningElement.style.display = methodConfig.fee > 0 ? 'none' : 'block';
...
window.openCartModal = function() {
  ...
  const palletWarn = document.getElementById('cart-pallet-warn');
  if (palletWarn) {
    palletWarn.style.display = totalWeight >= 20 ? 'block' : 'none';
  }
...
// 核心運費試算函式 (均以普貨標準計算)
// 計算計費重量 (海快：5.09kg 以下算 5kg，超過無條件進位；空運：1.09kg 以下算 1kg，超過無條件進位)
function getBilledWeight(weight, method) {
  if (weight <= 0) return 0;
  if (method === 'sea_fast') {
    if (weight <= 5.09) return 5;
    return Math.ceil(weight);
  } else if (method === 'air') {
    if (weight <= 1.09) return 1;
    return Math.ceil(weight);
  }
  return Math.ceil(weight);
}
...
function calculateShippingFee(weight, method) {
  const billedWeight = getBilledWeight(weight, method);
  if (billedWeight <= 0) return 0;
  
  if (method === 'sea_fast') {
    if (billedWeight <= 5) {
      return Math.round(360 + (billedWeight * 10));
    } else {
      return Math.round(billedWeight * 62);
    }
  } else if (method === 'air') {
    if (billedWeight <= 1) {
      return 250;
    } else {
      return Math.round(billedWeight * 150);
    }
  }
  return 0;
}
